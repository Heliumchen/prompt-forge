...
describe('detectVariables', () => {
  ...
  test('handles null/undefined content', () => {
    expect(detectVariables(null as any)).toEqual({ variables: [], hasVariables: false });
    expect(detectVariables(undefined as any)).toEqual({ variables: [], hasVariables: false });
  });
...
describe('processTemplate', () => {
  ...
  test('handles null/undefined variable values', () => {
    const template = 'Hello {{name}}, how are you?';
    const variables: Variable[] = [{ name: 'name', value: null as any }];
    const result = processTemplate(template, variables);
  ...
  test('handles null/undefined variables array', () => {
    const template = 'Hello {{name}}, how are you?';
    expect(processTemplate(template, null as any)).toBe('Hello , how are you?');
    expect(processTemplate(template, undefined as any)).toBe('Hello , how are you?');
  });
  ...
  test('handles null/undefined template', () => {
    const variables: Variable[] = [{ name: 'name', value: 'John' }];
    expect(processTemplate(null as any, variables)).toBe('');
    expect(processTemplate(undefined as any, variables)).toBe('');
  });
...
describe('isValidVariableName', () => {
  ...
  test('handles null/undefined input', () => {
    expect(isValidVariableName(null as any)).toBe(false);
    expect(isValidVariableName(undefined as any)).toBe(false);
  });
});
...
describe('sanitizeVariableName', () => {
  ...
  test('handles empty/null/undefined input', () => {
    ...
    expect(sanitizeVariableName(null as any)).toBe('');
    expect(sanitizeVariableName(undefined as any)).toBe('');
  });
